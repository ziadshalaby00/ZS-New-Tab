    ...
    function showConfirm(message, opts = {}) {
        ...
        return new Promise((resolve) => {
            ...
            modal.innerHTML = `
                ...
                <p class="hint" style="margin-bottom:18px; font-size:13px; color:var(--text);">${escapeHtml(message)}</p>
                <div class="actions">
                    <button class="cancel">${escapeHtml(cancelLabel)}</button>
                    <button class="save danger-fill">${escapeHtml(confirmLabel)}</button>
                </div>
            `;
...
window.gl = ZSCore.getLocalStorageSize;